...
from rapyuta_io.utils import UnauthorizedError, ConflictError, RetriesExhausted
...
class Client(object):
    ...
    def create_device(self, name: str, arch: str, os: str, codename: str):
        ...
        response = RestClient(url).method(HttpMethod.POST).headers(
            headers).execute(payload={"kind": "VIRTUAL", "name": name, "architecture": arch,
        ...
        if not response.ok:
            ...
            raise Exception("hwildevices: {}".format(err_msg))

        return munchify(data)

    def delete_device(self, id: int):
        url = f"{self._host}/device/{id}"
        headers = self._get_auth_header()
    ...
    def get_device(self, id: int):
        ...
        if not response.ok:
            err_msg = data.get('error')
            raise Exception("hwil_devices: {}".format(err_msg))

        return munchify(data)

    def execute_cmd(self, id: int, command: str):
        url = f"{self._host}/command/"
        headers = self._get_auth_header()
        response = RestClient(url).method(HttpMethod.POST).headers(
            headers).execute(payload={"kind": "VIRTUAL", "device_id": id,
                                      "command": command})
        ...
        if not response.ok:
            err_msg = data.get('error')
            raise Exception("hwildevice: command {}".format(err_msg))

        return munchify(data)

    def poll_till_device_ready(self, id: int, sleep_interval: int, retry_limit: int):
        ...
        msg = 'Retries exhausted: Tried {} times with {}s interval.'.format(retry_limit, sleep_interval)
    ...
    def _get_auth_header(self: Client) -> dict:
        headers = dict(Authorization=f'Basic {self._token}')
        return headers
