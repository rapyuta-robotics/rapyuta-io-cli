...
@click.option(
    ...
    default=24 * 90,
    show_default=True,
    help="Key validity in hours (must be a positive integer).",
)
@click.option(
    "--ephemeral",
    "ephemeral",
...
def preauthkey(
    ...
    via MDM without requiring interactive authentication. It is not bound to
    a single device, so one key can enroll multiple devices and survive
    reinstalls. By default the key is non-ephemeral (the node persists in the
    network after disconnecting) and valid for 90 days.
    ...
    try:
        ...
        # reusable=True mints a key that can enroll many devices (and survive
        # reinstalls) from a single MDM profile. throwaway is left at its
        # default; it only controls rio-side binding persistence, not the key.
        ...
        if print_all:
            url = binding.get("HEADSCALE_URL") or ""
            tag = binding.get("HEADSCALE_ACL_TAG") or ""
            missing = [
                name
                for name, val in (("HEADSCALE_URL", url), ("HEADSCALE_ACL_TAG", tag))
                if not val
            ]
            if missing:
                click.secho(
                    f"{Symbols.ERROR} Missing required binding fields for --print-all: {', '.join(missing)}",
                    fg=Colors.RED,
                )
                raise SystemExit(1)
            click.echo(f"Key:          {key}")
            click.echo(f"Login server: {url}")
            click.echo(f"ACL tag:      {tag}")
        else:
            click.echo(key)
