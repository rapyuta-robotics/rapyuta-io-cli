...
def inspect_device(
    ...
    try:
        ...
        data = make_device_inspectable(
            device.to_dict(), daemon.model_dump(exclude_none=True, by_alias=True)
        )
...
def make_device_inspectable(device: dict, daemon: Munch) -> dict:
    data = {}
    ...
    if daemon is not None and daemon.get("status") is not None:
        data["daemons_status"] = daemon.get("status")
