...
def delete_role(
    ...
    try:
        ...
        spinner.text = click.style("Failed to delete role(s): {}".format(e), Colors.RED)
    ...
    try:
        ...
        if failed_count == 0 and success_count == len(roles):
            spinner_text = click.style(
                "{} role(s) deleted successfully.".format(len(roles)), Colors.GREEN
            )
            ...
            spinner_text = click.style(
                "{}/{} roles deleted successfully".format(success_count, len(roles)),
                Colors.YELLOW,
            )
            spinner_char = click.style(Symbols.WARNING, Colors.YELLOW)
        ...
        spinner.text = click.style("Failed to roles devices: {}".format(e), Colors.RED)
        spinner.red.fail(Symbols.ERROR)
        raise SystemExit(1) from e
