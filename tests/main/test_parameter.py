...
class TestParameterUpload:
    ...
    def test_01_upload_all_parameter_trees(self, logged_in_user_from_config):
        ...
        runner, user = logged_in_user_from_config

        # First delete both trees to start clean
        runner.invoke(
            cli, ["parameter", "delete", self.tree_name_1, "--force"]
        )
        runner.invoke(
            cli, ["parameter", "delete", self.tree_name_2, "--force"]
        )
    ...
    def test_07_download_all_parameter_trees(self, logged_in_user_from_config):
        ...
        try:
            ...
            # Verify downloaded tree directories exist
            assert os.path.isdir(
                os.path.join(download_dir, self.tree_name_1)
            ), f"Downloaded tree '{self.tree_name_1}' directory should exist"
            assert os.path.isdir(
                os.path.join(download_dir, self.tree_name_2)
            ), f"Downloaded tree '{self.tree_name_2}' directory should exist"
        finally:
    ...
    def test_08_download_specific_tree_by_name(self, logged_in_user_from_config):
        ...
        try:
            ...
            # Verify only the specified tree was downloaded
            assert os.path.isdir(
                os.path.join(download_dir, self.tree_name_1)
            ), f"Downloaded tree '{self.tree_name_1}' directory should exist"
        finally:
    ...
    def test_09_download_verifies_file_content(self, logged_in_user_from_config):
        ...
        try:
            ...
            # Compare the original and downloaded directory trees
            original_tree = os.path.join(self.parameter_trees_dir, self.tree_name_1)
            downloaded_tree = os.path.join(download_dir, self.tree_name_1)

            comparison = filecmp.dircmp(original_tree, downloaded_tree)

            # There should be no files only on the left (original) side
            assert len(comparison.left_only) == 0, (
                f"Files missing from download: {comparison.left_only}"
            )

            # Check for matching files at the top level
            if comparison.common_files:
                match, mismatch, errors = filecmp.cmpfiles(
                    original_tree, downloaded_tree, comparison.common_files, shallow=False
                )
                assert len(mismatch) == 0, (
                    f"Downloaded files differ from originals: {mismatch}"
                )
        finally:
            shutil.rmtree(download_dir, ignore_errors=True)
    ...
    def test_12_diff_modified_trees(self, logged_in_user_from_config):
        ...
        assert "---" in result.output or "+++" in result.output or "deleted" in result.output or "new" in result.output, (
    ...
    def test_15_upload_nonexistent_path(self, logged_in_user_from_config):
        ...
        result = runner.invoke(
            ...
            [
                "parameter",
                "upload",
                "/tmp/nonexistent-param-dir-12345",
                "--force",
            ],
        )
    ...
    def test_18_delete_nonexistent_tree(self, logged_in_user_from_config):
        """Test deleting a non-existent tree succeeds (API is idempotent)."""
        runner, user = logged_in_user_from_config
    ...
    def test_21_upload_recreate_and_verify_updated_content(
        ...
        try:
            ...
            # Compare downloaded content with updated version
            updated_tree = os.path.join(
                self.parameter_trees_updated_dir, self.tree_name_1
            )
            downloaded_tree = os.path.join(download_dir, self.tree_name_1)

            comparison = filecmp.dircmp(updated_tree, downloaded_tree)

            # The downloaded tree should match the updated version, not the original
            if comparison.common_files:
                match, mismatch, errors = filecmp.cmpfiles(
                    updated_tree, downloaded_tree, comparison.common_files, shallow=False
                )
                assert len(mismatch) == 0, (
                    f"Downloaded files should match the updated version: {mismatch}"
                )
        finally:
            shutil.rmtree(download_dir, ignore_errors=True)
