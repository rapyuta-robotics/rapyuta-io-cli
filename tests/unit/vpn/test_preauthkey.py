...
class TestPreauthkeyCommand:
    ...
    def test_binding_is_not_throwaway(
        self, runner, mock_config, mock_vpn_enabled, mock_create_binding
    ):
        runner.invoke(preauthkey, obj=_make_ctx_obj())

        _, kwargs = mock_create_binding.call_args
        # throwaway is left at the create_binding default (not forced False)
        assert "throwaway" not in kwargs
    ...
    def test_zero_expiry_is_rejected(self, runner, mock_config):
        result = runner.invoke(preauthkey, ["--expiry", "0"], obj=_make_ctx_obj())

        assert result.exit_code != 0
        assert "0" in result.output  # click IntRange error mentions the value

    def test_negative_expiry_is_rejected(self, runner, mock_config):
        result = runner.invoke(preauthkey, ["--expiry", "-1"], obj=_make_ctx_obj())

        assert result.exit_code != 0

    def test_print_all_missing_url_exits_nonzero(
        self, runner, mock_config, mock_vpn_enabled
    ):
        binding_no_url = {**_FAKE_BINDING, "HEADSCALE_URL": ""}
        with patch("riocli.vpn.preauthkey.create_binding", return_value=binding_no_url):
            result = runner.invoke(preauthkey, ["--print-all"], obj=_make_ctx_obj())

        assert result.exit_code != 0
        assert "HEADSCALE_URL" in result.output

    def test_print_all_missing_tag_exits_nonzero(
        self, runner, mock_config, mock_vpn_enabled
    ):
        binding_no_tag = {**_FAKE_BINDING, "HEADSCALE_ACL_TAG": None}
        with patch("riocli.vpn.preauthkey.create_binding", return_value=binding_no_tag):
            result = runner.invoke(preauthkey, ["--print-all"], obj=_make_ctx_obj())

        assert result.exit_code != 0
        assert "HEADSCALE_ACL_TAG" in result.output
